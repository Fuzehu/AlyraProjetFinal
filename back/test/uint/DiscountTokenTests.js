...
describe('DiscountToken', function () {
    ...
    it('should have correct name and symbol', async function () {   // ok
    ...
    it('should revert if the mint function is called by non-admins addresses', async function () {
    ...
    it('should revert if the addAdminRights function is not called by the owner', async function () {  // new test
        await expect(discountToken.connect(user1).addAdminRights(user2.address)).to.be.revertedWith(
            'Ownable: caller is not the owner'
        );
    });

    it('should revert if the revokeAdminRights function is not called by the owner', async function () {  // new test
        await discountToken.connect(owner).addAdminRights(user1.address);
