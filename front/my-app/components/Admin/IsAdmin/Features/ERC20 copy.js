...
const ERC20 = () => {
    ...
    const addAdminRights = async (_addr) => {
        try {
            const { request } = await prepareWriteContract({address: contractAddress,abi: Contract.abi,functionName: "addAdminRights",args: [_addr],})
            await writeContract(request)
            setAdminAddresses([adminAddresses, _addr])
            toast({title: 'Success !',description: `${_addr} has been successfully registered as admin !`,status: 'success',duration: 5000,isClosable: true,})
        } catch (err) {
            console.log(err);
            toast({title: 'Error!',description: 'An error occured.',status: 'error',duration: 3000,isClosable: true,})
        }
    }
    const revokeAdminRights = async (_addr) => {
        try {
            const { request } = await prepareWriteContract({address: contractAddress,abi: Contract.abi,functionName: "revokeAdminRights",args: [_addr],})
            await writeContract(request)
            setAdminAddresses(adminAddresses.filter(address => address !== _addr))
            toast({title: 'Success !',description: `${_addr} has been successfully revoked as admin !`,status: 'success',duration: 5000,isClosable: true,})
        } catch (err) {
            console.log(err);
            toast({title: 'Error!',description: 'An error occured.',status: 'error',duration: 3000,isClosable: true,})
        }
    ...
    const mint = async (_to, _amount) => {
        try {
            const { request } = await prepareWriteContract({address: contractAddress,abi: Contract.abi,functionName: "mint",args: [_to, _amount],})
            await writeContract(request)
            setMintedTokensReceiver([...mintedTokensReceiver, {receiver: _to}])
            setMintedTokensAmount([...mintedTokensAmount, {amount: _amount}])
            toast({title: 'Success !',description: `${_amount} has been successfully transfered to ${_to} !`,status: 'success',duration: 5000,isClosable: true,})
        } catch (err) {
            console.log(err);
            toast({title: 'Error!',description: 'An error occured.',status: 'error',duration: 3000,isClosable: true,})
        }
    ...
    useEffect(() => {
        const getPastEvents = async () => {
            const addAdminRightsLogs = await client.getLogs({event: parseAbiItem('event AdminRightsGranted(address indexed newAdminAddress)'), fromBlock: 0n,toBlock: 'latest'})
            const revokeAdminRightsLogs = await client.getLogs({event: parseAbiItem('event AdminRightsRevoked(address indexed adminAddress)'),fromBlock: 0n, toBlock: 'latest'})
            const tokensMintedLogs = await client.getLogs({event: parseAbiItem('event TokensMinted(address indexed receiver, uint amount)'),fromBlock: 0n,toBlock: 'latest'})
    ...
    return (
        <Flex flexDirection="column" mt="5rem">
            ...
            <Flex flexDirection="row" width="100%" justifyContent="space-around">
                ...
                <Box width="50%" p={5} bg="#F3F2FF" border="3px solid darkslateblue" borderRadius="lg" mt={3} mb={3} ml={3}>
